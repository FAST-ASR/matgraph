...
class PIT_LFMMILoss(torch.nn.Module):
    ...
    def __init__(self, denfsm: FSM, den_scale=1.0):
        super().__init__()
        self.denfsm = denfsm
        self.den_scale = den_scale

    def forward(self, est_llhs: torch.Tensor, seqlengths: torch.Tensor, numfsms: List[BatchFSM]):
        """
        Args:
        ...
        assert est_llhs.size(1) == n_spkrs, f"Expected number of sources is {n_spkrs}, got {input.size(1)}"
